...
static void addDevice(const char deviceName[], size_t* selectedRow, std::vector<std::vector<std::string>>& configurationTable, const std::vector<std::string>& labelsCombo, const std::vector<std::vector<std::string>>& itemsCombo, std::vector<std::string>& selectedItemsCombo, const std::vector<std::string>& labelsInputInt, std::vector<int>& selectedItemsInputInt, const std::vector<std::string>& labelsInputText, std::vector<std::string>& selectedItemsInputText) {
	// Add change delete button
	addChangeDeleteButtons(deviceName);

	// If we pressed ADD button
	if (Tools_Gui_CreateDialogs_showPopUpComboInputIntInputTextMultiDialogOKCancle(("popUpAdd" + std::string(deviceName)).c_str(), labelsCombo, itemsCombo, selectedItemsCombo, labelsInputInt, selectedItemsInputInt, labelsInputText, selectedItemsInputText)) {
		std::vector<std::string> row;
		for (size_t i = 0; i < selectedItemsCombo.size(); i++) {
			row.push_back(selectedItemsCombo.at(i));
		}
		for (size_t i = 0; i < selectedItemsInputInt.size(); i++) {
			row.push_back(std::to_string(selectedItemsInputInt.at(i)));
		}
	...
	if (configurationTable.size() > 1) {
		// Change row
		if (Tools_Gui_CreateDialogs_showPopUpComboInputIntInputTextMultiDialogOKCancle(("popUpChange" + std::string(deviceName)).c_str(), labelsCombo, itemsCombo, selectedItemsCombo, labelsInputInt, selectedItemsInputInt, labelsInputText, selectedItemsInputText)) {
			std::vector<std::string> row;
			for (size_t i = 0; i < selectedItemsCombo.size(); i++) {
				row.push_back(selectedItemsCombo.at(i));
			}
	...
	if (ImGui::Begin("Configure measurement", configureMeasurement)) {
		if (ImGui::CollapsingHeader("CDC")) {
			...
			if (ImGui::CollapsingHeader("STM32 PLC")) {
				...
				static std::vector<int> selectedItemsInputInt(labelsInputInt.size());
			...
			if (ImGui::CollapsingHeader("Open SAE J1939")) {
				...
				std::vector<std::string> labelsInputInt;
				static std::vector<int> selectedItemsInputInt;
				...
				// Create the device
				addDevice("Open SAE J1939", &selectedRow, configurationTable, labelsCombo, itemsCombo, selectedItemsCombo, labelsInputInt, selectedItemsInputInt, labelsInputText, selectedItemsInputText);
		...
		if (ImGui::CollapsingHeader("Modbus")) {
			...
			if (ImGui::CollapsingHeader("ADL400")) {
				...
				// Create the device
				addDevice("ADL400", &selectedRow, configurationTable, labelsCombo, itemsCombo, selectedItemsCombo, labelsInputInt, selectedItemsInputInt, labelsInputText, selectedItemsInputText);
