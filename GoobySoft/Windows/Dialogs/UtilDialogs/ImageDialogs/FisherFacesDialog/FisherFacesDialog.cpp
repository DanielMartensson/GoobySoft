...
void Windows_Dialogs_UtilDialogs_ImageDialogs_FisherFacesDialog_showFisherFacesDialog(bool* fisherFacesDialog) {
	// Display
	...
	if (ImGui::Begin("Fisherfaces", fisherFacesDialog, ImGuiWindowFlags_NoResize)) {
		...
		/* Extract */
		MODEL_NN_SETTINGS* settings_fisherfaces = &parameterHolder->model_settings.settings_fisherfaces;
		...
		if (Tools_Gui_CreateDialogs_showPopUpConfirmDialogOKCancle("buildFisherModel", "Do you want to build model?", nullptr)) {
			/* Get data */
			MODEL* data_collect = fisherfaces(&parameterHolder->model_settings);
			
			/* Insert new data */
			static bool notFirstTime = false;
			for (int i = 0; i < data_collect->fisherfaces_model.total_models; i++) {
				/* Free data */
				if (notFirstTime) {
					std::free(parameterHolder->models.fisherfaces_model.model_b[i]);
					std::free(parameterHolder->models.fisherfaces_model.model_w[i]);
				}
			
				/* And then create data */
				parameterHolder->models.fisherfaces_model.model_b[i] = (float*)malloc(data_collect->fisherfaces_model.model_row[i] * sizeof(float));
				parameterHolder->models.fisherfaces_model.model_w[i] = (float*)malloc(data_collect->fisherfaces_model.model_row[i] * data_collect->fisherfaces_model.model_column[i] * sizeof(float));

				/* Copy over */
				std::memcpy(parameterHolder->models.fisherfaces_model.model_b[i], data_collect->fisherfaces_model.model_b[i], data_collect->fisherfaces_model.model_row[i] * sizeof(float));
				std::memcpy(parameterHolder->models.fisherfaces_model.model_w[i], data_collect->fisherfaces_model.model_w[i], data_collect->fisherfaces_model.model_row[i] * data_collect->fisherfaces_model.model_column[i] * sizeof(float));

				/* Update parameters */
				parameterHolder->models.fisherfaces_model.activation_function[i] = data_collect->fisherfaces_model.activation_function[i];
				parameterHolder->models.fisherfaces_model.model_column[i] = data_collect->fisherfaces_model.model_column[i];
				parameterHolder->models.fisherfaces_model.model_row[i] = data_collect->fisherfaces_model.model_row[i];
			}
			...
			/* Total models */
			parameterHolder->models.fisherfaces_model.total_models = data_collect->fisherfaces_model.total_models;
			
			/* Clear */
			imcollectfree(data_collect);
		}
