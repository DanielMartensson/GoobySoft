...
void Windows_Dialogs_ConfigurationDialogs_ConfigurationSTM32PLC_ConfigureDateTimeDialog_showConfigureDateTimeDialog(bool* configureDateTime) {
	...
	if (ImGui::Begin("Configure date time", configureDateTime, ImGuiWindowFlags_NoResize)) {
		// Get the USB ports
		std::string connectedUSBports = Tools_Hardware_USB_getConnectedPorts();

		// Create combo box
		static int usbIndex = 0;
		ImGui::PushItemWidth(100);
		ImGui::Combo("Connected USB ports", &usbIndex, connectedUSBports.c_str());
		char port[20];
		Tools_Software_Algorithms_extractElementFromCharArray(connectedUSBports.c_str(), usbIndex, port);
		...
		if (ImGui::Button("Receive date time")) {
			...
			const int32_t result = Tools_Hardware_USB_read(port, data, 6, 100, true, false);
			if (result > 0) {
				year = 2000 + data[0];
				month = data[1];
				date = data[2];
		...
		if (ImGui::Button("Receive week day alarm")) {
			...
			Tools_Hardware_USB_write(port, data, 1, 0);
			const int32_t result = Tools_Hardware_USB_read(port, data, 5, 100, true, false);
			if (result > 0) {
				week_day = data[0] + 1;
				hours = data[1];
				minutes = data[2];
