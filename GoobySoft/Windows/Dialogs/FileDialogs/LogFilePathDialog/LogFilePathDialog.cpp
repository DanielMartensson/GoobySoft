...
bool Windows_Dialogs_FileDialogs_LogFilePathDialog_showDeleteFileDialog(bool* deleteFileDialog) {
	if (Windows_Dialogs_FileDialogs_LogFilePathDialog_showSelectFileDialog(deleteFileDialog)) {
		...
		std::memset(Tools_Hardware_ParameterStore_getParameterHolder()->fileSettings.fileName, 0, sizeof(Tools_Hardware_ParameterStore_getParameterHolder()->fileSettings.fileName)); // Clear the file name as well
